...
double convertLength(double value, int from, int to) {
    double ratios[] = { 1.0, 0.01, 0.001, 1000.0, 1609.34 };
...
double convertVolume(double value, int from, int to) {
    double ratios[] = { 1.0, 0.001, 0.000001, 1000.0, 0.001, 1000000.0 };
    return value * ratios[from] / ratios[to];
}
...
int main() {
    ...
    do {
        ...
        if (choice == 1) {
            ...
            string units[] = { "кг", "г", "мг", "т" };
            double result = convertMass(value, from, to);
            cout << value << " " << units[from - 1] << " = " << result << " " << units[to - 1] << "\n";
        }
        else if (choice == 3) {
            int from, to;
            ...
            cout << "Выберите исходную единицу:\n"
                ...
                << "3. Микролитры\n"
                << "4. Кубические метры\n"
                << "5. Кубические сантиметры\n"
                << "6. Кубические миллиметры\n"
                << "Введите номер единицы: ";
            cin >> from;
            cout << "Выберите целевую единицу:\n"
                << "1. Литры\n"
            ...
            string units[] = { "л", "мл", "мкл", "м³", "см³", "мм³" };
            double result = convertVolume(value, from, to);
            cout << value << " " << units[from - 1] << " = " << result << " " << units[to - 1] << "\n";
        }
    } while (choice != 0);
